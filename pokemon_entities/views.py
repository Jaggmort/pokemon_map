...
def show_all_pokemons(request):
    ...
    entities = PokemonEntity.objects.filter(appeared_at__lte=local_time,
...
def show_pokemon(request, pokemon_id):
    ...
    if pokemon_db.parent:
        absolute_url = request.build_absolute_uri(pokemon_db.parent.image.url)
    ...
    if child:
        absolute_url = request.build_absolute_uri(child.image.url)
        pokemon['next_evolution'] = {"title_ru": child.title,
                                     "pokemon_id": child.pk,
                                     "img_url": absolute_url
                                     }
    local_time = localtime()
    entities = PokemonEntity.objects.filter(pokemon=pokemon_id,
    ...
    for entity in entities:
        ...
        add_pokemon(folium_map, entity.lat,
                    entity.lon,
                    absolute_url
                    )
